// A classical binary heap, but using reference tracking if the Val type derives from Heap::Entry
...
class Heap
{
    struct Entry
    {
        ...
        {
            if constexpr (std::is_base_of_v<Reference, Val>)
            {
...
public:
    ...
    Heap copyByOrder(const R1 &from, const R2 &to) const
    {
        ...
        auto itTo = to.begin();
        for (size_t i = 0; i < size(); i++,++itFrom,++itTo)
        {
    ...
    std::pair<Key, Val &> peek() { return {data.front().k, *data.front().v}; }
